...
asdf install tilt 0.33.21
asdf global tilt 0.33.21
```
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.33.21/tilt.0.33.21.mac.x86_64.tar.gz | tar -xzv tilt && \
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.33.21/tilt.0.33.21.linux.x86_64.tar.gz | tar -xzv tilt && \
  sudo mv tilt /usr/local/bin/tilt
```
...
```powershell
Invoke-WebRequest "https://github.com/tilt-dev/tilt/releases/download/v0.33.21/tilt.0.33.21.windows.x86_64.zip" -OutFile "tilt.zip"
