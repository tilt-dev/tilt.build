...
asdf install tilt 0.18.12
asdf global tilt 0.18.12
```
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.18.12/tilt.0.18.12.mac.x86_64.tar.gz | tar -xzv tilt && \
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.18.12/tilt.0.18.12.linux.x86_64.tar.gz | tar -xzv tilt && \
  sudo mv tilt /usr/local/bin/tilt
```
...
```powershell
Invoke-WebRequest "https://github.com/tilt-dev/tilt/releases/download/v0.18.12/tilt.0.18.12.windows.x86_64.zip" -OutFile "tilt.zip"
