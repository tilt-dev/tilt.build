...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.22.3/tilt.0.22.3.mac.x86_64.tar.gz | tar -xzv tilt && \
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.22.3/tilt.0.22.3.linux.x86_64.tar.gz | tar -xzv tilt && \
  sudo mv tilt /usr/local/bin/tilt
```
...
```powershell
Invoke-WebRequest "https://github.com/tilt-dev/tilt/releases/download/v0.22.3/tilt.0.22.3.windows.x86_64.zip" -OutFile "tilt.zip"
