...
asdf install tilt 0.37.3
asdf global tilt 0.37.3
```
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.37.3/tilt.0.37.3.mac.x86_64.tar.gz | tar -xzv tilt && \
...
```bash
curl -fsSL https://github.com/tilt-dev/tilt/releases/download/v0.37.3/tilt.0.37.3.linux.x86_64.tar.gz | tar -xzv tilt && \
  sudo mv tilt /usr/local/bin/tilt
```
...
```powershell
Invoke-WebRequest "https://github.com/tilt-dev/tilt/releases/download/v0.37.3/tilt.0.37.3.windows.x86_64.zip" -OutFile "tilt.zip"
